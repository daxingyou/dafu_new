...
<head>
    ...
    <link rel="stylesheet" href="/resources/css2/securityCenter.css">
 ...
 <div class="vip_info clearfix container">
    ...
    <div class="pull-right vip_info_pan">
        ...
        <div class="vip_info_content security_main">
            <div class="star">
                <?php
 $num = abs($schedule/100)*10/2; for($i=1;$i<6;$i++){ if($num-$i >= 0){ echo "<i class=\"iconfont special\">&#xe610;</i>"; }else{ echo "<i class=\"iconfont \">&#xe610;</i>"; } } ?>
            </div>
            <div class="text">
                <h3>您的账号安全级别为<?php echo ($aqjibie); ?>，可以通过完善安全信息提高级别。</h3>
                <p>上次登录： <?php echo ($_SESSION['lastlogin']['lasttime']); ?> , IP：<?php echo ($_SESSION['lastlogin']['lastip']); ?>，<?php echo ($_SESSION['lastlogin']['login_address']); ?> | 不是我登录？<a href="<?php echo U('Member/update_pass');?>" class="">修改密码</a></p>
            </div>
            <ul class="set_list">
                <li class="clearfix">
                    ...
                    <a class="set_btn pull-right" href="<?php echo U('Member/update_pass');?>">修改密码</a>
                </li>
                <li class="clearfix">

                    <span class="iconfont <?php if(!empty($userinfo["tradepassword"])): ?>success<?php endif; ?>">&#xe60f;</span>
                    <div class="set_list_column">
                        <strong>已设置资金密码</strong>
                        <em>资金密码用于提现、绑定银行卡等操作，可保障资金安全。</em>
                    </div>
                        <a class="set_btn pull-right" href="<?php echo U('Member/update_safepass');?>">修改资金密码</a>
                        <a class="set_btn pull-right" href="<?php echo U('Member/find_safepass');?>">找回资金密码</a>

                </li>
                <li class="clearfix">
                    <span class="iconfont <?php if(!empty($userinfo["tel"])): ?>success<?php endif; ?>">&#xe600;</span>
                    <div class="set_list_column ">
                        <strong><?php if(!empty($userinfo["tel"])): ?>已绑定密保手机<?php else: ?>未绑定密保手机<?php endif; ?></strong>
                        <em>密保手机可以增加账户安全性，快速找回帐号密码。</em>
                    </div>
                    <?php if(!empty($userinfo["tel"])): ?><a class="set_btn pull-right" href="<?php echo U('Member/safephone');?>">已绑定保手机</a>
                        <?php else: ?>
                        <a class="set_btn pull-right" href="<?php echo U('Member/safephone');?>">绑定密保手机</a><?php endif; ?>

                </li>
<!--                <li class="clearfix">
                    <span class="iconfont <?php if(!empty($userinfo["question"])): ?>success<?php endif; ?> ">&#xe605;</span>
                    <div class="set_list_column">
                        <strong><?php if(!empty($userinfo["question"])): ?>已设置密保问题<?php else: ?>未设置密保问题<?php endif; ?></strong>
                        <em>密保问题可以增加账户安全性，快速找回帐号密码。</em>
                    </div>
                    <?php if(!empty($userinfo["question"])): ?><a class="set_btn pull-right" href="<?php echo U('Member/updateProblem');?>">修改密保问题</a>
                        <?php else: ?><a class="set_btn pull-right" href="<?php echo U('Member/setProblem');?>">设置密保问题</a><?php endif; ?>
                </li>-->
                <li class="clearfix">
                    <span class="iconfont <?php if(!empty($userinfo["email"])): ?>success<?php endif; ?>">&#xe61d;</span>
                    <div class="set_list_column">
                        <strong><?php if(!empty($userinfo["email"])): ?>已绑定密保邮箱<?php else: ?>未绑定密保邮箱<?php endif; ?></strong>
                        <em>绑定邮箱可以增加账户安全性，快速找回帐号密码。</em>
                    </div>
                    <?php if(!empty($userinfo["email"])): ?><a class="set_btn pull-right" href="<?php echo U('Member/bindmail');?>">修改密保邮箱</a>
                        <?php else: ?>
                        <a class="set_btn pull-right" href="<?php echo U('Member/bindmail');?>">绑定密保邮箱</a><?php endif; ?>
